...
@section('content')
<div class="container-fluid py-4" id="margin-hub">
    <!-- Header Area -->
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            <h1 class="h3 text-white gradient-text font-weight-bold mb-1">Margin Pairs Hub</h1>
            <p class="text-muted small mb-0">Manage margin trading pairs, borrow rates, and leverages.</p>
        </div>
    ...
    <div class="glass-card satin-border overflow-hidden shadow-xl">
        ...
        <div class="table-responsive">
            <table class="table text-white table-hover align-middle" id="margin-table">
                 ...
                 <tbody>
                    @forelse($pairs as $pair)
                        ...
                        <tr data-quote="{{ $quote }}" data-id="{{ $pair->id }}">
                            ...
                            <td><div class="text-warning font-weight-bold">{{ number_format((float)($pair->borrow_rate_hourly * 100), 4) }}%</div></td>
                            <td><span class="badge badge-primary-glass px-2 py-1">{{ $pair->max_leverage ?? 10 }}x</span></td>
                            <td class="text-right">
                                <div class="d-flex justify-content-end gap-2">
                                    <form action="{{ url('admin/margin/pairs/delete/'.$pair->id) }}" method="GET" class="delete-pair-form">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted"> <i data-lucide="layers" class="mb-2 opacity-50" style="width:32px; height:32px;"></i> <br> No margin pairs available</td>
                        </tr>
                    @endforelse
                 </tbody>
            </table>
    ...
    <div class="modal-content glass-card satin-border" style="background: rgba(15,15,20,0.95);">
      <div class="modal-header border-bottom border-secondary p-4">
        <h5 class="modal-title text-white font-weight-bold d-flex align-items-center gap-2" id="addPairModalLabel">
            <i data-lucide="plus-circle" class="text-primary"></i> Add Margin Pair
        </h5>
      ...
      <div class="modal-body p-4">
        <form action="{{ route('admin.margin.pairs.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group mb-4">
                ...
                <input type="number" name="max_leverage" class="form-control glass-panel text-white border-0" value="10" required>
            </div>
            <div class="form-group mb-4">
                <label class="text-muted small text-uppercase font-weight-bold tracking-wider mb-2">Hourly Borrow Rate</label>
                <input type="number" step="0.0001" name="borrow_rate_hourly" class="form-control glass-panel text-white border-0" value="0.001">
            </div>
            <div class="form-group mb-4">
                <label class="text-muted small text-uppercase font-weight-bold tracking-wider mb-2">Maintenance Margin</label>
                <input type="number" step="0.0001" name="maintenance_margin" class="form-control glass-panel text-white border-0" value="0.05">
            </div>
            <div class="form-group mb-4">
...
<script>
    $(document).ready(function(){
        ...
        var marginTable = $('#margin-table').DataTable({
            "paging": true,
            "lengthChange": false,
            "searching": true,
            "ordering": true,
        ...
        $('#custom-search').on('keyup', function() {
            marginTable.search(this.value).draw();
        });
        ...
        $.fn.dataTable.ext.search.push(
            function(settings, data, dataIndex, rowData, counter) {
                if(settings.nTable.id !== 'margin-table') return true;
        ...
        $('#checkAll').on('change', function() {
            ...
            $('#margin-table tbody tr:visible .row-checkbox').prop('checked', isChecked);
            updateMassActionUI();
        });
        ...
        $('#btn-mass-delete').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Send AJAX request
                    $.ajax({
                        url: "{{ route('admin.margin.pairs.mass_delete') }}",
                        type: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                            ids: selectedIds
        ...
        $('#btn-sync-binance-header').on('click', function(e) {
            ...
            $.post("{{ route('admin.margin.pairs.sync_binance') }}", {
                _token: "{{ csrf_token() }}"
            }, function(res) {
