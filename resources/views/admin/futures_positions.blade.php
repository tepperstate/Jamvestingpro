...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Futures Positions Oversight</h1>
    
    <div class="row mb-4">
        <div class="col-12">
            <div class="card shadow p-4">
                <h5 class="mb-3"><i class="ri-settings-3-line"></i> Futures Auto-Approve Configuration</h5>
                <form action="{{ route('admin.settings.trading.auto-approve') }}" method="POST" class="d-flex align-items-center flex-wrap gap-3">
                    @csrf
                    <input type="hidden" name="futures_auto_approve_submit" value="1">
                    
                    <div class="form-check form-switch me-4">
                        <input class="form-check-input" type="checkbox" id="futures_auto_approve" name="futures_auto_approve" {{ site()->futures_auto_approve ? 'checked' : '' }}>
                        <label class="form-check-label" for="futures_auto_approve">Enable Auto-Approval</label>
                    </div>

                    <div class="input-group" style="max-width: 250px;">
                        <span class="input-group-text">Target Profit Margin</span>
                        <input type="number" step="0.01" class="form-control" name="futures_auto_win_percent" value="{{ site()->futures_auto_win_percent }}" placeholder="e.g. 20.00">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $position->margin_amount }}</td>
                        <td>{{ $position->unrealized_pnl }}</td>
                        <td>{{ $position->admin_status }}</td>
                        <td>
                            <form action="{{ url('admin/futures/positions/update') }}" method="POST">
                                @csrf
