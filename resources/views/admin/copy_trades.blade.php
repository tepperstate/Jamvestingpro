  ...
  <div class="container-fluid">
     <a onclick="history.back()" href="javascript:void">back</a>
      ...
      <div class="text-center justify-content-center mb-4">
         <h4 class="font-weight-bold m-0" style='color:var(--text-primary) !important'>Copy Trades</h4>
         <p>All Copy Trades.</p>
      </div>
...
<script>
   $(document).ready(function(){
      ...
      async function trade(){
        ...
        fetch("{{route('admin.all_copy_trades')}}",options)
        .then((res)=>res.json())
        .then((data)=>{
          let tr =""            
          ...
          tr 
              += `
                  <tr> 
                      ...
                      <td><code class="x-small text-white opacity-75">${val.trade_id}</code></td>
                      <td><div class="font-weight-bold text-white small">${val.user.first_name} ${val.user.last_name}</div></td>
                      <td class="small text-white">${val.trader_name}</td>
                      <td><span class="badge glass-panel text-white border-0 x-small">${val.symbol}</span></td>
                      <td class="small text-muted">${val.exchanges.name}</td>
                      <td><div class="font-weight-bold text-white small">${val.user.currency} ${parseFloat(val.amount).toFixed(2)}</div></td>
                      <td><div class="font-weight-bold text-success small">${val.user.currency} ${val.p_l}</div></td>
                      <td class="text-muted small">${val.expire_time}</td>
