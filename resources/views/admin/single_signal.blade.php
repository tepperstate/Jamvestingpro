  ...
  <div class="container-fluid">
     <a onclick="history.back()" href="javascript:void">back</a>
      ...
      <div class="text-center justify-content-center mb-4">
         <h4 class="font-weight-bold m-0" style='color:var(--text-primary) !important'>Single Signal</h4>
      </div>
         ...
         <div class='col-lg-12'>
            <div class="glass-card border-0 mb-4 shadow p-2">
               <div class="card-body">
                  <div class='row'>
                     <div class='col-lg-3'>
                        <h4 class='font-weight-bold' style='color:var(--text-primary) !important'>Single Signal</h4>
                      </p>
                     </div>
                     <div class='col-lg-9'>
                        <form method='post' action="{{route('edit_signal')}}" id="submit" class='row' enctype="multipart/form-data">
                           @csrf
                           <div class='col-lg-4 mt-1'>
                              <label class='font-text'>Signal Name</label>
                              <input type="hidden" name='id' value='{{$data->id}}'>
                              <input type='text' class='form-control' name='name' id="name" required value="{{$data->name}}">
                           </div>
                           <div class='col-lg-4 mt-1'>
                              <label class='font-text'>Signal Image</label>
                           ...
                           <div class='col-lg-4 mt-1'>
                              <label class='font-text'>Daily Trade</label>
                              <input type='text' class='form-control' name='daily' id="address" required value="{{$data->day}}">
                           </div>
                           <div class='col-lg-4 mt-1'>
                              <label class='font-text'>Min Profit (%)</label>
                              <input type='number' step="0.01" class='form-control' name='profit_min_percent' required value="{{$data->profit_min_percent ?? 5}}">
                           </div>
                           <div class='col-lg-4 mt-1'>
                              <label class='font-text'>Max Profit (%)</label>
                              <input type='number' step="0.01" class='form-control' name='profit_max_percent' required value="{{$data->profit_max_percent ?? 15}}">
                           </div>
