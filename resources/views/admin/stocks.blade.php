...
@section('title', 'Stock ETFs Management')
...
<div class="container-fluid py-4">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-5">
        <div>
            <h1 class="h2 mb-1 text-white gradient-text" style="font-weight: 800;">Stock ETFs Oversight</h1>
            <p class="text-muted mb-0">Configure Stock portfolios and manipulate individual user ROI.</p>
        </div>
        <div class="d-flex align-items-center gap-3">
             <div class="badge badge-info-glass px-4 py-2 border-0 satin-border" style="background: rgba(59, 130, 246, 0.05) !important;">
                <span class="text-white small font-weight-bold">ACTIVE Stock POSITIONS: {{ $investments->total() }}</span>
            </div>
        </div>
    </div>

    <!-- Stock Plans Configuration -->
    <div class="glass-card satin-border overflow-hidden shadow-2xl mb-5">
        <div class="p-4 d-flex justify-content-between align-items-center" style="background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--glass-border);">
            <div>
                <h3 class="h5 text-white mb-1 font-weight-bold">Global Stock Portfolios Configuration</h3>
                <p class="text-muted x-small mb-0">Adjust baseline ROI, duration, and minimum capital requirements.</p>
            </div>
            <div class="d-flex gap-2">
                <form action="{{ route('admin.stock_etfs.auto_populate') }}" method="POST" class="d-inline" onsubmit="return confirm('Auto-populate real Stock ETFs from the API provider?');">
                    @csrf
                ...
                <form action="{{ route('admin.stock_etfs.refresh_logos') }}" method="POST" class="d-inline" onsubmit="return confirm('Refresh all Stock logos from the API provider?');">
                    @csrf
                    ...
                    @foreach ($plans as $plan)
                        <tr>
                            ...
                            <td class="text-right">
                                ...
                                <form action="{{ route('admin.stock_etfs.plan.delete', $plan->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this Stock plan?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content glass-card satin-border text-white">
            <div class="modal-header border-0">
                <h5 class="modal-title font-weight-bold">Create Stock Portfolio</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.stock_etfs.plan.store') }}" method="POST">
                @csrf
...
<div class="modal fade" id="editPlanModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content glass-card satin-border text-white">
            <div class="modal-header border-0">
                <h5 class="modal-title font-weight-bold">Configure Stock Portfolio</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
        ...
        <div class="modal-content glass-card satin-border text-white">
            ...
            <div class="modal-body p-4">
                ...
                <div class="form-group mb-4">
                    ...
                    <p class="x-small text-info mt-1">This will change the final payout amount for this specific user's Stock.</p>
                </div>
                <div class="form-group mb-4">
...
<script>
    ...
    function savePlan() {
        ...
        $.ajax({
            url: "{{ route('admin.stock_etfs.plan.update') }}",
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                id: id,
    ...
    function updateInvestment(id, action) {
        if (action === 'delete' && !confirm('Are you sure you want to delete this Stock record?')) return;
        
        $.ajax({
            url: "{{ route('admin.stock_etfs.investment.update') }}",
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                id: id,
