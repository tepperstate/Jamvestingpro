    ...
    <div class="spot-grid-layout">
        ...
        <div class="d-flex flex-column gap-3" style="grid-column: 2; height: 100%;">
            ...
            <div class="binance-panel tradingdesk-panel">
                ...
                <div class="td-sub-tabs" style="overflow-x: auto; white-space: nowrap; flex-wrap: nowrap; padding-bottom: 5px;">
                    ...
                    <span class="td-sub-tab" onclick="setOrderType('stop_loss', this)">Stop Loss</span>
                    <span class="td-sub-tab" onclick="setOrderType('take_profit', this)">Take Profit</span>
                    <span class="td-sub-tab" onclick="setOrderType('trailing_stop', this)">Trailing Stop</span>
                    <span class="td-sub-tab" onclick="setOrderType('oco', this)">OCO</span>
                </div>
                ...
                <div class="td-columns">
                    ...
                    <div class="td-column">
                        ...
                        <!-- Stop-Limit / Stop Loss / Take Profit Trigger Price Input (hidden by default) -->
                        <div class="td-input-group" id="buy-trigger-group" style="display: none;">
                            <span class="td-input-label">Stop</span>
                        ...
                        <!-- OCO / Stop-Limit Extra Limit Price Input (hidden by default) -->
                        <div class="td-input-group" id="buy-limit-group" style="display: none;">
                            <span class="td-input-label">Limit</span>
                            <input type="number" step="any" class="td-input-field" id="buy-limit-price" placeholder="Limit Price">
                            <span class="td-input-suffix">USDT</span>
                        </div>

                        <!-- Trailing Stop Delta Input (hidden by default) -->
                        <div class="td-input-group" id="buy-delta-group" style="display: none;">
                            <span class="td-input-label">Delta</span>
                            <input type="number" step="any" class="td-input-field" id="buy-trailing-delta" placeholder="0.00" min="0" max="100">
                            <span class="td-input-suffix">%</span>
                        </div>
                        ...
                        <button class="td-action-btn buy-btn" id="buy-btn-label" onclick="submitSpotOrder('buy')">Buy BTC</button>
                    </div>
                    ...
                    <div class="td-column">
                        ...
                        <!-- OCO / Stop-Limit Extra Limit Price Input (hidden by default) -->
                        <div class="td-input-group" id="sell-limit-group" style="display: none;">
                            <span class="td-input-label">Limit</span>
                            <input type="number" step="any" class="td-input-field" id="sell-limit-price" placeholder="Limit Price">
                            <span class="td-input-suffix">USDT</span>
                        </div>

                        <!-- Trailing Stop Delta Input (hidden by default) -->
                        <div class="td-input-group" id="sell-delta-group" style="display: none;">
                            <span class="td-input-label">Delta</span>
                            <input type="number" step="any" class="td-input-field" id="sell-trailing-delta" placeholder="0.00" min="0" max="100">
                            <span class="td-input-suffix">%</span>
                        </div>
                        ...
                        <button class="td-action-btn sell-btn" id="sell-btn-label" onclick="submitSpotOrder('sell')">Sell BTC</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr class="spot-order-row" data-id="{{ $o->id }}" data-symbol="{{ $o->symbol }}" data-type="{{ $o->type }}" data-price="{{ $o->price }}" data-amount="{{ $o->amount }}" data-ordertype="{{ $o->order_type }}" data-trigger="{{ $o->trigger_price }}" data-status="{{ $o->status }}" style="background: rgba(255,255,255,0.01); border-radius: 4px;">
                            ...
                            <td class="py-2 border-0 text-end">
                                <span id="sim-status-{{ $o->id }}" class="ledger-badge-status {{ $o->status == 'pending' ? 'pending' : ($o->status == 'approved' ? 'approved' : 'rejected') }}">
                                    @if($o->status == 'pending' && $o->order_type == 'market')
                                        OPEN POSITION
...
<script>
    ...
    function selectCryptoPair(id, symbol, price, name, changes) {
        ...
        let heldObj = userHoldings[cleanBase];
    ...
    function setOrderType(type, el) {
        ...
        $('#buy-limit-group, #sell-limit-group').hide();
        $('#buy-delta-group, #sell-delta-group').hide();
        ...
        if (type === 'market') {
            ...
            $('#buy-trigger-group, #sell-trigger-group').show(); // Optional activation price
            $('#buy-delta-group, #sell-delta-group').show();
        } else if (type === 'oco') {
            $('#buy-price-group, #sell-price-group').show(); // Main Price
            $('#buy-trigger-group, #sell-trigger-group').show(); // Stop Price
            $('#buy-limit-group, #sell-limit-group').show(); // Limit Price
            $('#buy-price').attr('placeholder', 'Price');
            $('#sell-price').attr('placeholder', 'Price');
        } else {
    ...
    function submitSpotOrder(side) {
        ...
        if (['stop-limit', 'stop_loss', 'take_profit', 'trailing_stop', 'oco'].includes(currentOrderType)) {
            let triggerPrice = parseFloat($(`#${side}-trigger-price`).val()) || 0;
            if (triggerPrice <= 0 && currentOrderType !== 'trailing_stop') {
                toastr.error('Please enter a valid stop/trigger price.', 'MISSING STOP PRICE');
                return;
            }
        }
        ...
        // For market, stop_loss, take_profit, trailing_stop orders, use current price for 'total' estimations
        // Limit and OCO orders use the set price.
        let orderPrice = ['market', 'stop_loss', 'take_profit', 'trailing_stop'].includes(currentOrderType) ? currentPrice : price;
        ...
        let orderPayload = {
            ...
            type: side,
            amount: mappedAmount,
            price: currentPrice, // Current market price at execution time
        ...
        if (['stop-limit', 'stop_loss', 'take_profit', 'trailing_stop', 'oco'].includes(currentOrderType)) {
            orderPayload.stop_price = parseFloat($(`#${side}-trigger-price`).val()) || null;
        }

        if (['stop-limit', 'limit', 'oco'].includes(currentOrderType)) {
            orderPayload.limit_price = price;
        }

        if (currentOrderType === 'oco') {
            orderPayload.limit_price = parseFloat($(`#${side}-limit-price`).val()) || 0; // stop-limit leg price
            orderPayload.price = price; // The standard limit leg price is sent as price here since backend SpotOrderController maps it if we send price but wait backend will accept limit_price. We should send price for the limit leg
        }

        if (currentOrderType === 'trailing_stop') {
            orderPayload.trailing_delta = parseFloat($(`#${side}-trailing-delta`).val()) || null;
        }
