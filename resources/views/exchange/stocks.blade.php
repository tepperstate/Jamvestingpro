...
<div class="container-fluid py-4" style="max-width: 1600px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5" data-aos="fade-down">
        <div>
            <div class="d-flex align-items-center gap-3">
                <h1 class="outfit display-5 mb-1 text-white">Stocks & ETFs</h1>
                <span id="live-indicator" class="badge bg-secondary text-white" style="font-size: 10px; letter-spacing: 1px;"><i class="ri-pulse-line me-1"></i>CONNECTING...</span>
            </div>
            <p class="text-secondary mb-0">Overview of notes regarding your investment</p>
        </div>
        <div class="d-flex align-items-center gap-4">
            <div class="input-group search-glass" style="width: 300px;">
                <span class="input-group-text bg-transparent border-0 text-secondary"><i class="ri-search-2-line"></i></span>
                <input type="text" id="global-stock-search" class="form-control bg-transparent border-0 text-white" placeholder="Search stocks (ex. GOGL)   ⌘+K">
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-sm btn-premium-outline"><i class="ri-export-line"></i> Export</button>
                <div class="dropdown">
                    <button class="btn btn-sm btn-premium-outline" data-toggle="dropdown"><i class="ri-more-2-fill"></i></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="glass-card-premium p-4" data-aos="fade-up" data-aos-delay="200" style="border-radius: 28px;">
        <!-- Market Navigation -->
        <div class="market-navigation d-flex gap-4 border-bottom border-white border-opacity-10 mb-4 overflow-auto pb-2" style="scrollbar-width: none;">
            <a href="{{ route('stocks.trade', ['market' => 'nasdaq', 'type' => $current_type]) }}" class="market-tab {{ $current_market == 'nasdaq' ? 'active' : 'text-muted' }}" data-market="nasdaq">NASDAQ GLOBAL</a>
            <a href="{{ route('stocks.trade', ['market' => 's&p500', 'type' => $current_type]) }}" class="market-tab {{ $current_market == 's&p500' ? 'active' : 'text-muted' }}" data-market="s&p500">S&P 500 COMPOSITE</a>
            <a href="{{ route('stocks.trade', ['market' => 'dow', 'type' => $current_type]) }}" class="market-tab {{ $current_market == 'dow' ? 'active' : 'text-muted' }}" data-market="dow">DOW JONES INDUSTRIALS</a>
            <a href="{{ route('stocks.trade', ['market' => 'nyse', 'type' => $current_type]) }}" class="market-tab {{ $current_market == 'nyse' ? 'active' : 'text-muted' }}" data-market="nyse">NYSE LISTED</a>
        </div>
    ...
    function submitTrade() {
        ...
        .then(result => {
            ...
            if(ok && data.status) {
                ...
                // Enforce sitewide full page reload on trade as requested
