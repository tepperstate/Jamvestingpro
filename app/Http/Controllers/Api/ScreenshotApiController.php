...
{
    /**
     * Generate structured JSON data exactly like the admin screenshot tool,
     * OR return a full HTML rendered view of the screenshot if 'render' is true.
     */
    public function generate(Request $request)
    {
        // 1. Validate the exact same parameters as admin generator
        $request->validate([
            ...
            'render_html' => 'nullable|boolean', // If true, return HTML instead of JSON
        ]);
        ...
        if ($request->user_id) {
            ...
            $user = (object) [
                ...
                'last_name' => '',
                'email' => 'api-generated@system.internal',
            ];
        }
        ...
        // Core Generation Logic replicating ScreenshotController
        switch ($request->type) {
            case 'trades':
                if ($request->user_id) {
        ...
        // If the API request prefers rendered HTML (for screenshot headless browsers)
        if ($request->render_html == true || strtolower($request->render_html) === 'true') {
            return view('api.screenshot_render', [
                'user' => $user,
                'type' => $request->type,
                'data' => $data,
        ...
        return response()->json([
            ...
            'data' => $data,
            'generated_at' => $generated_at,
            'render_url' => url('/api/screenshot/generate').'?'.http_build_query(array_merge($request->all(), ['render_html' => true])),
        ]);
    }
}
